...
#functions that will be used to check if the value inputed is correct or not
...
def menu():
    ...
    layout = [
        ...
        [sg.Text('Please enter your Food(ex:Egg, Chicken, Coffee)\n Note: Food is Measured in Quantity of 100g'), sg.Input(key='-FOOD-')], 
        [sg.Submit(), sg.Button('Clear'), sg.Button('Show Current Entries'),sg.Exit()]
    ]


    #Step 2, actually create the window
    window= sg.Window('Calorie Tracker Input Menu', layout, resizable=True)
    ...
    while True:
        ...
        if event == 'Submit':
           ...
           elif not check_food_input(values['-FOOD-']) or not get_food_nutrients(values['-FOOD-']):
            sg.popup('Food Input is in incorrect format or not a food')
...
# just to show csv file in terminal too 
def show():
      try:
